...
namespace Content.Client._Eclipse.Chemistry.UI
{
    ...
    public sealed partial class ChemMasterBotanyWindow : FancyWindow
    {
        ...
        public event Action? OnEjectGrinderChamberAll;
        public event Action<EntityUid>? OnEjectGrinderChamber;
        ...
        public ChemMasterBotanyWindow()
        {
            ...
            Tabs.SetTabTitle(2, Loc.GetString("chem-master-botany-window-grinder-tab"));

            InputEjectButton.OnPressed += _ => OnEjectBeaker?.Invoke();

            ChamberContentBox.EjectButton.OnPressed += _ => OnEjectGrinderChamberAll?.Invoke();
        ...
        {
            OnEjectGrinderChamber?.Invoke(_chamberVisualContents[args.ItemIndex]);
        }
        ...
        public void UpdateState(BoundUserInterfaceState state)
        {
            ...
            UpdateGrinderInfo(castState.BufferReagents, _entityManager.GetEntityArray(castState.ChamberContents));
        }

        private void UpdateGrinderInfo(IReadOnlyList<ReagentQuantity>? reagents, IReadOnlyList<EntityUid> containedSolids)
        {
        ...
        private void UpdateDosageFields(ChemMasterBotanyBoundUserInterfaceState castState)
        {
        ...
        /// </summary>
        /// <param name="state">State data sent by the server.</param>
        private string GenerateLabel(ChemMasterBotanyBoundUserInterfaceState state)
        {
        ...
        private void UpdatePanelInfo(ChemMasterBotanyBoundUserInterfaceState state)
        {
        ...
        public void HandleMessage(BoundUserInterfaceMessage message)
        {
            switch (message)
            {
                case ReagentGrinderWorkStartedMessage workStarted:
                    ...
                    BufferTransferButton.Disabled = true;
                    BufferDiscardButton.Disabled = true;
                    CreatePillButton.Disabled = true;
                    CreateBottleButton.Disabled = true;
                    for (uint i = 0; i < PillTypeButtons.Length; i++)
                    {
                        PillTypeButtons[i].Disabled = true;
                    }
                    break;
                case ReagentGrinderWorkCompleteMessage:
                    ...
                    BufferTransferButton.Disabled = false;
                    BufferDiscardButton.Disabled = false;
                    CreatePillButton.Disabled = false;
                    CreateBottleButton.Disabled = false;
                    for (uint i = 0; i < PillTypeButtons.Length; i++)
                    {
                        PillTypeButtons[i].Disabled = false;
                    }
                    break;
            }
        }
