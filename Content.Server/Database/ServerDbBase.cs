...
""");
        ...
        public async Task<OwnedVesselRecord?> GetOwnedShuttle(Guid player, int vesselId)
        {
            ...
            var entity = await db.DbContext.OwnedShuttles
                .Where(x => x.PlayerUserId == player && x.ShuttleId == vesselId)
                .SingleOrDefaultAsync();

            if (entity is null)
                return null;

            return MakeOwnedVesselRecord(entity);
        }

        public async Task<List<OwnedVesselRecord>> GetOwnedShuttlesForPlayer(Guid player)
        {
            await using var db = await GetDb();
            return await GetOwnedShuttlesForPlayerImpl(db, player);
        }

        protected async Task<List<OwnedVesselRecord>> GetOwnedShuttlesForPlayerImpl(DbGuard db, Guid player)
        {
            var entities = await db.DbContext.OwnedShuttles
                .Where(w => w.PlayerUserId == player)
                .Include(shuttle => shuttle.Player)
                .ToListAsync();

            return entities.Select(MakeOwnedVesselRecord).ToList();
        }

        private OwnedVesselRecord MakeOwnedVesselRecord(OwnedShuttles entity)
        {
            return new OwnedVesselRecord(entity.ShuttleId, entity.ShuttlePrototypeId, entity.ShuttleName, entity.ShuttleDescription, entity.ShuttleSavePrice, new ResPath(entity.ShuttlePath).ToRootedPath());
        }
