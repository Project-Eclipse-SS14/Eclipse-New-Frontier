# Appraisal cartridge
appraisal-program-name = Приложение ОценкаПлюс
appraisal-label-name = Предмет
appraisal-label-price = Предполагаемая цена

# Ledger cartridge
ledger-program-name = Приложение ФинансыПлюс
ledger-header = Доходы и расходы
ledger-total-income = Общий доход
ledger-no-income = Нету
ledger-total-expenses = Общие расходы
ledger-no-expenses = Нету
ledger-balance = Баланс:
ledger-no-balance = N/A

ledger-tab-Frontier = Аванпост Фронтир
ledger-tab-Nfsd = ДСБФ
ledger-tab-Medical = Медицинская диспетчерская
# Fallback, if this displays, something's wrong.
ledger-tab-Unknown = Неизвестно

ledger-entry-type-TickingIncome = Субсидии NT
ledger-entry-type-VendorTax = Налоги с продаж
ledger-entry-type-CargoTax = Налоги с перевозок
ledger-entry-type-MailDelivered = Почтовые выплаты
ledger-entry-type-BlackMarketAtmTax = Благотворители
ledger-entry-type-BlackMarketShipyardTax = Перепродажа лома
ledger-entry-type-BluespaceReward = Выплата за службу
ledger-entry-type-MedicalBountyTax = Медицинские выплаты
ledger-entry-type-AntiSmugglingBonus = Борьба с контрабандой
ledger-entry-type-StationDepositFines = Начисленные штрафы
ledger-entry-type-StationDepositDonation = Пожертвования
ledger-entry-type-StationDepositAssetsSold = Проданные активы
ledger-entry-type-StationDepositOther = Депозиты
ledger-entry-type-MailPenalty = Почтовые штрафы
ledger-entry-type-ShuttleRecordFees = Оплата проезда
ledger-entry-type-StationWithdrawalPayroll = Зароботная плата
ledger-entry-type-StationWithdrawalWorkOrder = Рабочие заказы
ledger-entry-type-StationWithdrawalSupplies = Затраты на поставку
ledger-entry-type-StationWithdrawalBounty = Награды
ledger-entry-type-StationWithdrawalOther = Другие расходы
